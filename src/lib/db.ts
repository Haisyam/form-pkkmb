...
// List of 30 official mentor candidates with exact NPMs
...
function seedDatabase() {
  ...
  if (groupCount.count === 0) {
    ...
    const groupsData = Array.from({ length: 30 }, (_, i) => {
      ...
      return {
        name: `Kelompok ${num}`,
        desc: `Pendampingan Mahasiswa Baru PKKMB UNMA 2026/2027 Kelompok ${num}`,
      };
    });
    insertMany(groupsData);
  }

  // Clear existing old dummy students & replace with exact 30 mentors list
